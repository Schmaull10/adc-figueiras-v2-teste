...
def fetch(url: str, debug_name: str = "page") -> str:
    """Load the public page through a normal headless Chromium session.

    Zerozero currently returns HTTP 403 to simple server-side requests from
    GitHub Actions. This uses a standards-compliant browser instead. It does
    not solve CAPTCHAs or bypass authentication/access controls.
    """
    debug_dir = ROOT / "debug-zerozero"
    ...
    if status and status >= 400:
        raise RuntimeError(f"HTTP {status} em Chromium (título: {title!r})")
    if len(html) < 500:
...
def main() -> int:
    ...
    try:
        competition_html = fetch(config["competition_url"], "competition")
    ...
    games = old.get("games", [])
    try:
        team_html = fetch(config["team_url"], "team")
